...
We also need to be careful to check that variables included in the model take more than one value. In R, using functions such as `glm`, these variables would be dropped automatically. However, this in not that case in `sparklyr`. 
...
glm_singular <- ml_generalized_linear_regression(rescue_cat, 
                                 ...
                                 link = "logit")
```
In this case, running the code above will produce an error since the `typeofincident` column only contains one variable, `Special Service`. The regression model will not run unless `typeofincident` is dropped from the formula above.  

There are some variables in this dataset which depend on others such that they always take a singular value for a given value of those other variables. For example, if we take a closer look at `total_cost` we can see it always takes a value that is equal to `hourly_cost` multiplied by `job_hours`:
...
Because of the method used, standard errors will not be produced, and therefore confidence intervals can’t be calculated. As a result, although the regression will run, attempting to access the coefficients and standard errors using the `tidy` or `ml_summary` methods described above will return an error. These 'singular' variables therefore need to be identified and removed from the analysis prior to using `ml_` functions.
...
```
<!-- Keep this for now because there does seem to be an issue with the ohe variables -->
<!-- One final point to note on singularity is that as a consequence of `singular` variables not being automatically removed by `ml_` functions, we cannot specify the `formula` argument simply using `is_cat ~ .` as we can with `glm`. Here, `.` is used as a shorthand for all variables in the dataset. Unfortunately, since this includes the target variable `is_cat`, the `ml_` functions will again revert to an optimisation method including all of the variables and fail to produce standard errors: -->

<!-- ```{r, eval = FALSE} -->
<!-- # Select only the variables we want to use in our regression -->
<!-- rescue_cat_selection <- rescue_cat %>%  -->
...
<!-- glm_singular <- ml_generalized_linear_regression(rescue_cat_selection,  -->
<!--                                  formula = "is_cat ~ .",  -->
<!--                                  family = "binomial",  -->
<!--                                  link = "logit") -->
